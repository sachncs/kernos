...
class Fuse:
    ...
    def __init__(self, cglobal: float = 1.0, clocal: float = 1.0, gateval: float = 0.5, gatelogit: float = 0.0, nofreeze: bool = False) -> None:
        self.cglobal = cglobal
        self.clocal = clocal
        self.gateval = gateval
        self.gate = Gate(gatelogit=gatelogit, nofreeze=nofreeze)

    def forward(self, phig: Array, phil: Array, cglobal: float | None = None, clocal: float | None = None, gateval: float | None = None) -> Array:
        ...
        return np.concatenate([np.sqrt(gateval) * phig / cglobal, np.sqrt(1.0 - gateval) * phil / clocal], axis=1)
